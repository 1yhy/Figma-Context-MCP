...
function printDetectionResult(result: IconDetectionResult): void {
  ...
  const size = result.size
    ? `(${Math.round(result.size.width)}x${Math.round(result.size.height)})`
    : "";
...
function testSingleNodeDetection(node: FigmaNode): void {
  ...
  // 去重
  const uniqueNodes = testNodes.filter(
...
function testExpectedResults(node: FigmaNode): void {
  ...
  for (const expected of expectedResults) {
    ...
    const result = detectIcon(testNode);
    const isCorrect =
      result.shouldMerge === expected.shouldMerge &&
    ...
    if (isCorrect) {
      ...
      console.log(
        `   期望: shouldMerge=${expected.shouldMerge}, format=${expected.exportFormat || "any"}`,
      );
