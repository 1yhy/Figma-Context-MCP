...
// 统计 px 值的精度
function countPxPrecision(jsonStr: string): { highPrecision: number; lowPrecision: number; examples: string[] } {
...
async function main() {
  ...
  console.log(`   原始数据 - 高精度值: ${originalPrecision.highPrecision}, 低精度值: ${originalPrecision.lowPrecision}`);
  if (originalPrecision.examples.length > 0) {
    console.log(`   原始高精度示例: ${originalPrecision.examples.join(', ')}`);
  }
  console.log(`   简化数据 - 高精度值: ${simplifiedPrecision.highPrecision}, 低精度值: ${simplifiedPrecision.lowPrecision}`);
  ...
  const textDefaultsPassed =
    simplifiedTextDefaults.textAlignLeft === 0 &&
    simplifiedTextDefaults.verticalAlignTop === 0;
  results.push({
    name: '文本默认值省略',
  ...
  console.log(`   justifyContent:"flex-start" 出现次数: ${simplifiedFlexDefaults.justifyContentStart}`);
